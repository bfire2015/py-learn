...
# -*- coding: utf-8 -*-
"""
__title__ = '68、SVM算法5个代码案例_加深理解SVM算法原理_笔记3'
__author__ = 'BfireLai'
__mtime__ = '2018/7/24'
"""
...
plt.xticks(x_tmp, ['linear-svm', 'rbf-svm', 'poly-svm', 'sigmoid-svm'], rotation=0)
plt.grid(b=True)
...
plt.suptitle('鸢尾花数据SVM分类器不同核函数模型比较', fontsize=18, color='r')
plt.show()
...
plt.xlim(x1_min, x1_max)
plt.ylim(x2_min, x2_max)
plt.title('鸢尾花数据rbf-svm分类', fontsize=16)
plt.grid(b=True, ls=':')
plt.tight_layout(pad=1.5)
...
plt.xlim(x1_min, x1_max)
plt.ylim(x2_min, x2_max)
plt.title('鸢尾花数据poly-svm分类', fontsize=16)
plt.grid(b=True, ls=':')
plt.tight_layout(pad=1.5)
...
plt.xlim(x1_min, x1_max)
plt.ylim(x2_min, x2_max)
plt.title('鸢尾花数据sigmoid-svm分类', fontsize=16)
plt.grid(b=True, ls=':')
plt.tight_layout(pad=1.5)
